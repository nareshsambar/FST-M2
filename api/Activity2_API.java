...
public class Activity2_API {
  ...
  Response response = given().contentType(ContentType.JSON).when().get(BASE_URL+"/nareshsambar");
  ...
  response.then().body("username", equalTo("nareshsambar"));
  response.then().body("firstName", equalTo("Naresh"));
  response.then().body("lastName", equalTo("Sambar"));
  response.then().body("email", equalTo("something@mail.com"));
  response.then().body("password", equalTo("password123"));
  response.then().body("phone", equalTo("9812763450"));
}
@Test(priority = '3')
	public void deleteReq() {
		Response response = given().contentType(ContentType.JSON).when().pathParam("username", "nareshsambar")
				.delete(BASE_URL + "/{username}");
		// Assertion
		  System.out.println(response.asPrettyString());
		response.then().body("code", equalTo(200));
		response.then().body("message", equalTo("nareshsambar"));
	}
}